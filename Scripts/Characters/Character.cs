...
public class Character : MonoBehaviour {
	...
	public float lives;
	public float damage;
	public float attackInterval = 1f; // seconds between discrete hits
	...
	void Update(){
		...
		//find closest enemy
		if(currentTarget == null || currentTargetCharacter == null || currentTargetCharacter.lives <= 0){
			findCurrentTarget();	
		}
		else{
			walkRandomly = false;
    ...
    foreach(GameObject potentialTarget in enemies){
        ...
        if(Vector3.Distance(transform.position, potentialTarget.transform.position) < closestDistance && potentialTarget != null){
			...
			if(!currentTarget || (currentTarget && Vector3.Distance(transform.position, currentTarget.position) > 2)){
				...
				currentTargetCharacter = currentTarget.GetComponent<Character>();
			}
        }
    }	
	}
	...
	private void TryAttackCurrentTarget(){
		...
		currentTargetCharacter.lives -= damage;
		attackTimer = attackInterval;
	}

	private void TryAttackCastle(){
		if(castleComponent == null) return;
		if(attackTimer > 0f) return;
		castleComponent.lives -= damage;
		attackTimer = attackInterval;
	}
