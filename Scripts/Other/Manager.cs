...
public class Manager : MonoBehaviour {
	[Header("Performance")]
	[SerializeField] private float castleRefreshInterval = 0.1f; // seconds between castle scans
	[SerializeField] private float unitsRefreshInterval = 0.25f; // seconds between unit count scans
	...
	private Text unitsLabelText;

	private Text playerCastleStrengthText;
	private Text enemyCastleStrengthText;
	private Image playerCastleStrengthBar;
	private Image enemyCastleStrengthBar;
	...
	private float time;
	private float castleTimer;
	private float unitsTimer;
	...
	void Awake(){
		...
		unitsLabelText = unitsLabel.GetComponentInChildren<Text>();
		
		playerCastleStrengthText = GameObject.Find("Player castle strength text").GetComponent<Text>();
		enemyCastleStrengthText = GameObject.Find("Enemy castle strength text").GetComponent<Text>();
		playerCastleStrengthBar = GameObject.Find("Player castle strength bar").GetComponent<Image>();
		enemyCastleStrengthBar = GameObject.Find("Enemy castle strength bar").GetComponent<Image>();
		...
		missionText = missionPanel.transform.Find("Mission text").gameObject.GetComponent<Text>();
	...
	void Start(){
		...
		time = 0;
		castleTimer = 0f; // force immediate first update
		unitsTimer = 0f;
	}
	...
	void Update(){
		...
		if(castleTimer <= 0f){
			castleTimer = castleRefreshInterval;
			GetCastleStrength();
			UpdateCastleUI();
			CheckGameEndStates();
		}
		...
		if(unitsTimer <= 0f){
			unitsTimer = unitsRefreshInterval;
			UpdateUnitCounts();
		}
		...
		//set victory based on mission (evaluated on each castle update)
		...
		if(fading && startMenuCanvasGroup.alpha > 0){
			startMenuCanvasGroup.alpha -= Time.deltaTime * fadespeed;
		}
		else if(startMenuCanvasGroup.alpha <= 0){
	...
	private void CheckGameEndStates(){
	...
	private void UpdateCastleUI(){
		playerCastleStrengthText.text = "" + (int)playerCastleStrength;
		enemyCastleStrengthText.text = "" + (int)enemyCastleStrength;
		enemyCastleStrengthBar.fillAmount = enemyCastleStrength/enemyCastleStrengthStart;
		playerCastleStrengthBar.fillAmount = playerCastleStrength/playerCastleStrengthStart;
	}
	...
	private void UpdateUnitCounts(){
		...
		unitsLabelText.text = "Player Units: " + playerUnits + "  |  Enemy Units: " + enemyUnits + "  |  Selected Units: " + selectedUnits;
	}
	...
	public void startGame(){
		...
		if(characterManager != null){
			characterManager.addCharacterButtons();
		}
	}
